...
def test_info_on_homepage(app, db):
    if len(db.get_contact_list()) == 0:
        app.contact.create(Contact(firstname="NewName"))
    ...
    for i in range(len(contacts_from_ui)):
        ...
        assert contacts_from_ui[i].all_phones_from_homepage == merge_phone_like_on_homepage(contacts_from_db[i])
        assert contacts_from_ui[i].all_emails_from_homepage == merge_emails_like_on_homepage(contacts_from_db[i])
